 ...
 *       reuses its name. Entity ids are server-generated, and a recreated definition draws a fresh
 *       id, so the token no longer describes it.
 * </ul>
 *
...
public final class TagVersionToken {
  ...
  /**
   * Returns the definition's entity id, so a server-side caller can find the definition a token was
   * issued for even after its name changed. The token stays opaque to clients: this is not an
   * id-addressed API, and this value does not reach a response.
   */
