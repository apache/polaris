 ...
 *
 * <p>It resolves a {@link ServiceIdentityInfoDpo} into a {@link ServiceIdentityCredential} using the
 * {@link ServiceIdentityProvider}, then uses the provided authentication parameters to generate
...
public class DefaultPolarisCredentialManager implements PolarisCredentialManager {
  ...
  public DefaultPolarisCredentialManager(ServiceIdentityProvider serviceIdentityProvider) {
  ...
  public @Nonnull EnumMap<ConnectionCredentialProperty, String> getConnectionCredentials(
    ...
    Optional<ServiceIdentityCredential> serviceIdentityCredential =
        serviceIdentityProvider.getServiceIdentityCredential(serviceIdentity);
    if (serviceIdentityCredential.isEmpty()) {
  ...
  @VisibleForTesting
  public StsClient getStsClient(@NotNull AwsIamServiceIdentityCredential awsIamCredential) {
    // TODO: Use STS client pool to reduce client creation overhead
    StsClientBuilder stsClientBuilder = StsClient.builder();
    stsClientBuilder.credentialsProvider(awsIamCredential.getAwsCredentialsProvider());
