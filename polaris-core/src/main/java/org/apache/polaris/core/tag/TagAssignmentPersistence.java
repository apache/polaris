...
public interface TagAssignmentPersistence {
  ...
  /**
   * Atomically delete the specified tag definition entity together with its assignment records, but
   * only while every assignment record it still holds is one the caller already classified. This is
   * the persistence capability behind dropping a definition whose surviving assignments a caller
   * has judged inert: that judgement needs information this layer does not have, so the caller
   * makes it and this method refuses to act on it once it has gone stale.
   *
   * <p>Implementations re-read the definition's assignment rows inside the same atomicity boundary
   * as the deletes and compare their identities against {@code classifiedAssignments}. A row
   * outside that set was written after the caller classified, was never judged, and may be live, so
   * the whole operation changes nothing and answers false. A classified row that has since
   * disappeared is not a conflict: the caller's judgement covered it, and removing fewer rows than
   * it saw still leaves none of the definition's rows behind.
   *
   * <p>Implementations must also prevent a row from being written between that re-read and the
   * commit, because a check the writer does not see is not a check.
   *
   * <p>Identity membership alone is not enough, so each entry also carries the state of its target
   * as the caller observed it, and implementations require that state to still hold. A row is inert
   * because of something about its target, and a target can change in a way that makes the same row
   * live again without changing the row: restoring a table schema that still contains a dropped
   * column makes a column row on that field current once more, and an assignment written afterwards
   * replaces the same identity rather than adding one. Comparing identities could not see that.
   *
   * @param callCtx call context
   * @param tagEntity the tag definition entity to delete together with its assignment records
   * @param classifiedAssignments every assignment row the caller classified, each with the target
   *     state it was judged from; an empty set requires the definition to hold no assignment row at
   *     all
   * @return true when the definition and its assignment records were deleted; false when the
   *     caller's classification could not be honoured, because an unclassified assignment row was
   *     present, because a referenced target moved on from the state it was judged in, or because
   *     the definition itself changed concurrently. Either way nothing was changed and the caller
   *     may classify again
   */
  default boolean deleteTagAndClassifiedAssignmentRecords(
      @NonNull PolarisCallContext callCtx,
