...
public class FeatureConfiguration<T> extends PolarisConfiguration<T> {
  ...
  public static final FeatureConfiguration<Boolean> ENABLE_OPENLINEAGE_INGEST =
      PolarisConfiguration.<Boolean>builder()
          ...
          .description(
              "If true, the OpenLineage ingest feature is enabled: its endpoints are advertised to "
                  + "clients in the catalog configuration response during endpoint discovery. The "
                  + "routes are always mounted when the OpenLineage extension is assembled into the "
                  + "server, so this flag is the switch that turns the feature on or off for "
                  + "clients.")
          .defaultValue(true)
          .buildFeatureConfiguration();
