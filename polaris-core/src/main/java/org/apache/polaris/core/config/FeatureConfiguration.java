      ...
      PolarisConfiguration.<Boolean>builder()
          ...
          .description(
              "If set to true, read-only AWS credential vending policies may reference SSE-KMS keys owned by a different AWS account in the same region. "
                  + "This broadens the KMS resource from the storage account to all KMS keys in the region and grants the KMS permissions required for encryption and decryption, including kms:Encrypt, kms:GenerateDataKey, kms:Decrypt, and kms:DescribeKey, through the vended credentials. "
