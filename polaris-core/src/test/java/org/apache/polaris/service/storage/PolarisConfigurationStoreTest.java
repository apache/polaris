...
public class PolarisConfigurationStoreTest {
  ...
  public void testConfigsCanBeCastedFromString() {
    ...
    for (PolarisConfiguration<?> c : configs) {
      Assertions.assertEquals(c.defaultValue, store.getConfiguration(realm, c));
    }
  }
  ...
  public void testInvalidCastThrowsException() {
    ...
    PolarisConfigurationStore store =
        new PolarisConfigurationStore() {
          @SuppressWarnings("unchecked")
          @Override
          public <T> T getConfiguration(Realm realm, String configName) {
            return (T) "abc123";
          }
        };

    for (PolarisConfiguration<?> c : configs) {
      Assertions.assertThrows(NumberFormatException.class, () -> store.getConfiguration(realm, c));
    }
  }
