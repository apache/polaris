...
class GcpAuthenticationParametersDpoTest {
  ...
  void testSerializeAndDeserialize() throws Exception {
    ...
    ConnectionConfigInfoDpo.DEFAULT_MAPPER.writeValue(writer, dpo);
    String json = new String(baos.toByteArray(), UTF_8);
    GcpAuthenticationParametersDpo deserialized =
        ConnectionConfigInfoDpo.DEFAULT_MAPPER.readValue(
            json, GcpAuthenticationParametersDpo.class);
    assertEquals(dpo, deserialized);
  }
