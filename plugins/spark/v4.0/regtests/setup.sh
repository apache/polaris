...
EOF
  if [[ "$TABLE_FORMAT" == "hudi" ]]; then
    cat << EOF >> ${SPARK_CONF}
spark.jars.packages org.apache.polaris:polaris-spark-4.0_$SCALA_VERSION:$POLARIS_VERSION,org.apache.hudi:hudi-spark4.0-bundle_${SCALA_VERSION}:1.1.1
...
EOF
    ...
    cat << EOF >> ${SPARK_CONF}
spark.jars.packages org.apache.polaris:polaris-spark-4.0_$SCALA_VERSION:$POLARIS_VERSION,io.delta:delta-spark_${SCALA_VERSION}:4.0.1
EOF
  fi
else
  cat << EOF >> ${SPARK_CONF}
...
EOF
  if [[ "$TABLE_FORMAT" == "hudi" ]]; then
    cat << EOF >> ${SPARK_CONF}
spark.jars.packages org.apache.hudi:hudi-spark4.0-bundle_${SCALA_VERSION}:1.1.1
EOF
  else
    cat << EOF >> ${SPARK_CONF}
spark.jars.packages io.delta:delta-spark_${SCALA_VERSION}:4.0.1
EOF
  fi
fi
