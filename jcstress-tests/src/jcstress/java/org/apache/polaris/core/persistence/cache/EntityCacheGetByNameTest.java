...
public class EntityCacheGetByNameTest {
  ...
  @Description(
      "Tests getOrLoadByName is thread-safe.  In this test, two actors are calling getOrLoadByName "
          ...
          + "two calls to getOrLoadByName.  Expected behaviour is that the two actors get the same "
          + "object twice.  Only a single database call should happen.  There should be no race "
          + "condition between threads for the same entity name.  And the cache should never go "
