 ...
 *
 * <p>Runs against a full Polaris server that has the {@code polaris-extensions-openlineage} module
 * assembled in. The server is spawned by the {@code polaris-server-test-runner} plugin and resolved
 * through {@code ExternalPolarisServerManager} (registered via {@code META-INF/services} in this
 * source set). The reusable client helpers ({@link OpenLineageApi}, {@link PolarisClient}) live in
 * {@code polaris-tests}; the contract itself lives here because OpenLineage ingest is an optional
 * extension, not part of the normative Polaris server surface.
 *
...
public class OpenLineageServiceIT {
