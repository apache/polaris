...
public class TestCatalogMaintenance {
  ...
  public void catalogMaintenance() {
    ...
    var callCtx =
        new PolarisCallContext(realmContext, session, polarisDiagnostics, configurationStore);
    ...
    // Ensure that "object created before references are bumped" are not purged by maintenance.
    ...
    var currentReferenceHeads =
        Stream.concat(realmReferenceNames(), References.catalogReferenceNames(catalogId))
            .toList()
            .stream()
  ...
  private static void mandatoryCatalogObjsForTestImpl(Persistence persistence, long catalogId) {
            ...
            CatalogStateObj.builder()
                ...
                .changes(
                    newUpdatableIndex(persistence, CHANGE_SERIALIZER)
                        .toIndexed("", (x, o) -> fail()))
                .locations(
                    newUpdatableIndex(persistence, LONG_VALUES_SERIALIZER)
                        .toIndexed("", (x, o) -> fail()))
    ...
    persistence.updateReferencePointer(
        persistence.fetchReference(
            References.perCatalogReferenceName(
                CatalogStateObj.CATALOG_STATE_REF_NAME_PATTERN, catalogId)),
    ...
    persistence.updateReferencePointer(
        persistence.fetchReference(
            References.perCatalogReferenceName(
                CatalogGrantsObj.CATALOG_GRANTS_REF_NAME_PATTERN, catalogId)),
  ...
  private static void mandatoryRealmObjsForTestImpl(Persistence persistence) {
    var catalogsObj =
        persistence.write(
            ...
            CatalogsObj.class);
    persistence.updateReferencePointer(
        persistence.fetchReference(CatalogsObj.CATALOGS_REF_NAME), objRef(catalogsObj));
    ...
    persistence.updateReferencePointer(
        persistence.fetchReference(ImmediateTasksObj.IMMEDIATE_TASKS_REF_NAME),
    ...
    persistence.updateReferencePointer(
        persistence.fetchReference(PolicyMappingsObj.POLICY_MAPPINGS_REF_NAME),
