...
import static org.apache.polaris.persistence.nosql.metastore.TypeMapping.initializeCatalogIfNecessary;
...
import static org.apache.polaris.persistence.nosql.metastore.TypeMapping.polarisPrincipalSecretsToPrincipal;
...
class PersistenceMetaStore implements BasePersistence, IntegrationPersistence {
  ...
  <T extends PolarisEntity & LocationBasedEntity> Optional<String> hasOverlappingSiblings(
    ...
    return memoizedIndexedAccess
        ...
        .flatMap(
            catalogStateObj -> {
              var locationsIndex =
                  catalogStateObj.locations().indexForRead(persistence, LONG_VALUES_SERIALIZER);
              var byId =
  ...
  private <I, T> Page<T> listEntitiesAsPage(
      long catalogStableId,
      long parentId,
  ...
  ListEntitiesResult listEntities(
    ...
    return new ListEntitiesResult(listedPage);
  }
  ...
  public PolarisPrincipalSecrets generateNewPrincipalSecrets(
    ...
    return performPrincipalChange(
        ...
        (state, ref, byName, byId, byClientId) -> {
          ...
          updatedPrincipalBuilder.id(generateNewId()).updateTimestamp(now);
  ...
  PolarisPrincipalSecrets loadPrincipalSecrets(String clientId) {
