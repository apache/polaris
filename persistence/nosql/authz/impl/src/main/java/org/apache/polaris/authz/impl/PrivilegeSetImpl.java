...
/**
 * Represents a set of {@link Privilege}s, implemented using a bit-map.
 *
 ...
 */
final class PrivilegeSetImpl implements PrivilegeSet {
  private final Privileges privileges;
  private final byte[] bytes;

  private PrivilegeSetImpl(Privileges privileges, PrivilegeSetBuilderImpl builder) {
    this.bytes = builder.bitSet.toByteArray();
    this.privileges = privileges;
  }

  PrivilegeSetImpl(Privileges privileges, byte[] bytes) {
    this.bytes = bytes;
    this.privileges = privileges;
  }
  ...
  public boolean contains(Object privilege) {
    return privilege instanceof Privilege && contains((Privilege) privilege);
  }
  ...
  @Override
  public boolean containsAll(Collection<?> privileges) {
  ...
  static final class PrivilegeSetBuilderImpl implements PrivilegeSetBuilder {
    ...
    public PrivilegeSetBuilder addPrivileges(@Nonnull PrivilegeSet privilegeSet) {
      var bytes =
          (privilegeSet instanceof PrivilegeSetImpl)
              ? ((PrivilegeSetImpl) privilegeSet).bytes
