...
class VertxAsyncExec implements AsyncExec, AutoCloseable {
  ...
  private final class CancelableFuture<R> implements Cancelable<R>, Runnable {
    ...
    void schedule(long delayMillis) {
      if (delayMillis > 0) {
        timerId = vertx.setTimer(delayMillis, this::execAsync);
      } else {
        execAsync(-1L);
      }
    }

    void periodic(long initialDelayMillis, long delayMillis) {
      timerId = vertx.setPeriodic(initialDelayMillis, delayMillis, this::execAsync);
      periodic = true;
    }
