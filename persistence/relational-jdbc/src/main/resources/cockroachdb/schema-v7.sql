...
-- CockroachDB schema v7 (matching PostgreSQL schema v7)
...
--    tag_assignment_record
--  * Compatible with PostgreSQL wire protocol

-- Changes from v6:
--  * Added the `tag_assignment_record` table and its two tag-side indexes for tag assignments
--    (the column storing the selected value is named `tag_value`: `value` is a reserved word in H2)
...
CREATE TABLE IF NOT EXISTS tag_assignment_record (
    realm_id TEXT NOT NULL,
    target_catalog_id BIGINT NOT NULL,
    target_id BIGINT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_tag_assignment_record_by_tag ON tag_assignment_record (realm_id, tag_catalog_id, tag_id);
CREATE INDEX IF NOT EXISTS idx_tag_assignment_record_by_tag_value ON tag_assignment_record (realm_id, tag_catalog_id, tag_id, tag_value);
