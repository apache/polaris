...
internal fun generateDigest(input: File, output: File, algorithm: String) {
    ...
    input.inputStream().use {
        ...
        output.writeText(md.digest().joinToString(separator = "") { eachByte -> "%02x".format(eachByte) })
    }
}

internal fun fetchAsfProject(asfName: String): Pair<Map<String, Any>, Boolean> {
    val tlpPrj: Map<String, Any>? = parseJson("https://projects.apache.org/json/projects/$asfName.json")
    return if (tlpPrj != null) {
        Pair(tlpPrj, false)
    } else {
        val podlings: Map<String, Map<String, Any>> = parseJson("https://projects.apache.org/json/foundation/podlings.json")!!
...
internal fun <T: Any> parseJson(url: String): T? {
    try {
        @Suppress("UNCHECKED_CAST")
        return JsonSlurper().parse(URI(url).toURL()) as T
