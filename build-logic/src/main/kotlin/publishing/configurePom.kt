 ...
 *
 * For all projects except the root project, the pom gets the `<license>`, `<name>`, `<description>`, `<parent>`
 * elements and fixes dependencies in `<dependencyManagement>` to be consumable by Maven.
 *
 * The root project generates the parent pom, containing all the necessary elements to pass Sonatype validation
 * and some more information like `<developers>` and `<contributors>`. Most of the information is taken from publicly
 * consumable Apache project information from `https://projects.apache.org/json/projects/<project-name>>.json`.
 * `<developers>` contains all (P)PMC members and committers from that project info JSON, ordered by real name.
 * `<contributors>` is taken from GitHub's `https://api.github.com/repos/apache/<project-name>/contributors?per_page=1000`
 * endpoint to give all contributors credit, ordered by number of contributions (as returned by that endpoint).
 */
internal fun configurePom(project: Project, mavenPublication: MavenPublication, task: Task) = mavenPublication.run {
    ...
    pom {
        if (project != project.rootProject) {
            ...
            // Add the license to every pom to make it easier for downstream project to retrieve the license.
            licenses {
                license {
            ...
            task.doFirst {
                ...
                inceptionYear.set((asfPrj["created"] ?: asfPrj["started"]!!).toString().replace("(\\d+)-.*", "\\1"))
                ...
                scm {
                    val codeRepo: String = if (asfPrj.contains("repository")) {
                ...
                issueManagement {
                    url.set(if (asfPrj.contains("repository")) {
 ...
 */
fun addContributorsToPom(mavenPom: MavenPom, asfName: String, asfProjectName: String) = mavenPom.run {
    contributors {
 ...
 */
fun addDevelopersToPom(mavenPom: MavenPom, asfName: String, e: PublishingHelperExtension, fromPodlings: Boolean) = mavenPom.run {
    developers {
        ...
        if (!fromPodlings) {
            ...
            filteredPeople = people.filter { entry ->
...
fun addMissingMandatoryDependencyVersions(project: Project, projectNode: Node) {
    xmlNode(xmlNode(projectNode, "dependencyManagement"), "dependencies")?.children()?.forEach {
        ...
        if (xmlNode(dependency, "version") == null) {
            ...
            if (depResult == null) {
                depResult =
                    findDependency(project.configurations.findByName("testRuntimeClasspath"), depGroup, depName)
            }
