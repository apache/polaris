...
internal fun configureOnRootProject(project: Project) = project.run {
    ...
    releaseEmailTemplate.configure {
        group = "publishing"
        description = "Generate release-vote email subject + body, including the staging repository URL, if run during the Maven release."
        ...
        doFirst {
            ...
            val stagingRepositoryUrlRegistryRegistration = gradle.sharedServices.registrations.named<BuildServiceRegistration<StagingRepositoryDescriptorRegistryBuildService, *>>("stagingRepositoryUrlRegistry")
            val staginRepoUrl = if (stagingRepositoryUrlRegistryRegistration.isPresent) {
                val stagingRepositoryUrlRegistryBuildServiceRegistration = stagingRepositoryUrlRegistryRegistration.get()
                val stagingRepositoryUrlRegistryService = stagingRepositoryUrlRegistryBuildServiceRegistration.getService()
    ...
    if (isRelease) {
        sourceTarball.configure {
            finalizedBy(releaseEmailTemplate)
        }
    }
