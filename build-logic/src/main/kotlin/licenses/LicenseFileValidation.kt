...
class LicenseFileValidation : DependencyFilter {
  companion object {
    ...
    val licenseMentionPrefix = "* Maven group:artifact IDs: "
    val separator =
      "--------------------------------------------------------------------------------"
  }
  ...
  override fun filter(data: ProjectData?): ProjectData {
    ...
    val superfluousDependencies =
      licenseFile
        .lines()
        .filter { it.startsWith(licenseMentionPrefix) }
        .map { it.substring(licenseMentionPrefix.length) }
    ...
    data.allDependencies.forEach { mod ->
      ...
      val groupModuleRegex =
        "^* Maven group:artifact IDs: $groupModule$".toRegex(RegexOption.MULTILINE)
