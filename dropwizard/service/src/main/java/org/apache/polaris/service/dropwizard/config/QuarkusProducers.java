...
import org.apache.polaris.service.dropwizard.auth.QuarkusAuthenticatorConfiguration;
import org.apache.polaris.service.dropwizard.catalog.io.QuarkusCatalogIOConfiguration;
import org.apache.polaris.service.dropwizard.context.QuarkusContextConfiguration;
import org.apache.polaris.service.dropwizard.persistence.QuarkusPersistenceConfiguration;
import org.apache.polaris.service.dropwizard.ratelimiter.QuarkusRateLimiterConfiguration;
...
public class QuarkusProducers {
  ...
  public RealmContextResolver realmContextResolver(
      QuarkusContextConfiguration config,
      @Any Instance<RealmContextResolver> realmContextResolvers) {
    return realmContextResolvers
        .select(Identifier.Literal.of(config.realmContextResolver().type()))
        .get();
  }

  @Produces
  public FileIOFactory fileIOFactory(
      QuarkusCatalogIOConfiguration config, @Any Instance<FileIOFactory> fileIOFactories) {
  ...
  public Authenticator<String, AuthenticatedPolarisPrincipal> authenticator(
      QuarkusAuthenticatorConfiguration config,
      @Any Instance<Authenticator<String, AuthenticatedPolarisPrincipal>> authenticators) {
    return authenticators.select(Identifier.Literal.of(config.type())).get();
  }
  ...
  public IcebergRestOAuth2ApiService icebergRestOAuth2ApiService(
    ...
    return services.select(Identifier.Literal.of(config.service().type())).get();
  }
  ...
  public TokenBrokerFactory tokenBrokerFactory(
    ...
    return tokenBrokerFactories
        .select(Identifier.Literal.of(config.tokenBrokerFactory().type()))
        .get();
  }
