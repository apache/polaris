...
public class DropwizardServerManager implements PolarisServerManager {
  public static final String TEST_REALM =
      "POLARIS"; // referenced in polaris-server-integrationtest.yml
  public static final String SERVER_CONFIG_PATH = "polaris-server-integrationtest.yml";

  @Override
  public Server realmForContext(ExtensionContext context) {
    try {
      return new Holder();
    } catch (IOException e) {
  ...
  private static class Holder implements Server {
    ...
    public Holder() throws IOException {
      ...
      token = new AuthToken(getToken(), "root");
    ...
    private String getToken() {
      return TokenUtils.getTokenFromSecrets(
          ext.client(),
          baseUri().toString(),
