...
public class InternalAuthenticationMechanism implements HttpAuthenticationMechanism {
  ...
  public int getPriority() {
    // Must be higher than the default OIDC authentication mechanism, which has priority
  ...
  protected boolean shouldProcess(RoutingContext context) {
    return configuration.type() == AuthenticationType.INTERNAL
        || configuration.type() == AuthenticationType.MIXED;
  }

  /** Decodes the token and verifies its signature. */
  protected DecodedToken decodeToken(String credential) {
    return tokenBroker.verify(credential);
  }
