...
curl -i -X POST -H "Authorization: Bearer ${SPARK_BEARER_TOKEN}" -H 'Accept: application/json' -H 'Content-Type: application/json' \
  ...
  -d '{"name": "spark_sql_views_catalog", "id": 100, "type": "INTERNAL", "readOnly": false, "properties": {"default-base-location": "file:///tmp/spark_sql_s3_catalog", "drop-with-purge.enabled": "true"}, "storageConfigInfo": {"storageType": "FILE", "allowedLocations": ["file:///tmp/spark_sql_s3_catalog"]}}' > /dev/stderr
