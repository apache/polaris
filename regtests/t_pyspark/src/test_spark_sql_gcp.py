...
def gcp_credentials():
  """
  Fixture that retrieves the GCP credentials from the environment variables.
...
@pytest.fixture
def catalog(root_client, catalog_client, gcs_test_base):
  """
  ...
  Args:
    ...
    gcs_test_base: The base path for GCS.

  Yields:
    Catalog: The created test catalog.
  """
  storage_conf = GcpStorageConfigInfo(
    storage_type="GCS",
    allowed_locations=[f"{gcs_test_base}/polaris_test/spark_sql_gcp_catalog/"]
  )
  catalog_name = 'spark_sql_gcp_catalog'
  catalog = Catalog(
    name=catalog_name,
    type='INTERNAL',
    properties={
      "default-base-location": f"{gcs_test_base}/polaris_test/{catalog_name}"
    },
...
@pytest.mark.skipif(os.environ.get('GCS_TEST_ENABLED', 'False').lower() != 'true',
                    reason='GCS_TEST_ENABLED is not set or is false')
