...
@pytest.fixture
def catalog(root_client, catalog_client, azure_dfs_test_base):
  """
  ...
  Args:
    ...
    azure_dfs_test_base: The base path for Azure DFS.
  ...
  storage_conf = AzureStorageConfigInfo(
    storage_type="AZURE",
    allowed_locations=[f"{azure_dfs_test_base}/polaris_test/{catalog_name}/"],
    tenant_id=tenant_id
  )
  catalog = Catalog(
    name=catalog_name,
    type='INTERNAL',
    properties={
      "default-base-location": f"{azure_dfs_test_base}/polaris_test/{catalog_name}"
    },
