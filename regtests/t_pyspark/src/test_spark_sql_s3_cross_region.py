...
def test_bucket():
  """
  Fixture that retrieves the cross region AWS S3 bucket name from the environment variables.

  Returns:
    str: The cross region AWS S3 bucket.
  """
  return os.getenv('AWS_CROSS_REGION_BUCKET')

@pytest.fixture
def aws_role_arn():
  """
   Fixture that retrieves the AWS role ARN for the cross region AWS S3 bucket from the environment variables.

  Returns:
    str: The AWS role ARN for the cross region AWS S3 bucket.
  """
  return os.getenv('AWS_ROLE_FOR_CROSS_REGION_BUCKET')
...
def catalog(root_client, catalog_client, test_bucket, aws_role_arn):
  ...
  catalog_name = 'spark_sql_s3_cross_region_catalog'
  catalog = Catalog(
    name=catalog_name,
    type='INTERNAL',
    properties={
...
@pytest.mark.skipif(os.environ.get('AWS_CROSS_REGION_TEST_ENABLED', 'False').lower() != 'true',
                    reason='AWS_CROSS_REGION_TEST_ENABLED is not set or is false')
