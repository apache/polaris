...
| rateLimiter | object | `{"enabled":false,"maxRequests":9999,"period":"PT1S"}` | Polaris rate limiter configuration. |
| rateLimiter.enabled | bool | `false` | Whether to enable rate limiting. |
| rateLimiter.maxRequests | int | `9999` | The maximum number of requests allowed per period, for each realm. |
| rateLimiter.period | string | `"PT1S"` | The time period for the rate limit. |
