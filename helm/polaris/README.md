...
| rateLimiter | object | `{"tokenBucket":{"requestsPerSecond":1000,"type":"default","window":"PT10S"},"type":"no-op"}` | Polaris rate limiter configuration. |
| rateLimiter.tokenBucket | object | `{"requestsPerSecond":1000,"type":"default","window":"PT10S"}` | The configuration for the default rate limiter, which uses the token bucket algorithm with one bucket per realm. |
...
| rateLimiter.type | string | `"no-op"` | The type of rate limiter to use. Two built-in types are supported: default and no-op. |
...
| tasks | object | `{"maxConcurrentTasks":100,"maxQueuedTasks":1000}` | Polaris task queue configuration. |
