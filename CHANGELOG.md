...
- Iceberg metrics reporting no longer fails the client request when persistence or resolution fails.
  A failed write is logged and the report dropped (previously this could surface as HTTP 500 on
  `POST .../metrics`). Reports are also dropped with a warning when the resolved leaf is missing or
  not `TABLE_LIKE` (for example passthrough-facade tables with no local table entity), so a bogus
  `table_id` is never persisted; that drop applies to all configured reporters, including log-only.
