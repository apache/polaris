...
- Added the default-disabled `ALLOW_CROSS_ACCOUNT_KMS_KEYS` feature flag for AWS credential vending. When enabled, read-only policies may reference SSE-KMS keys owned by another AWS account in the same region. This broadens `kms:Decrypt` and `kms:DescribeKey` access to a cross-account regional wildcard, so operators should enable it only with intentional cross-account KMS permissions and trust relationships.
