...
- Authorization failure messages (HTTP 403 / `ForbiddenException` from `PolarisAuthorizerImpl`) now name the specific missing privilege(s) and the entity each was checked against, e.g. `... is not authorized for op CREATE_TABLE_DIRECT; missing TABLE_CREATE on NAMESPACE 'ns1'`. Secondary-side failures are labeled `(secondary)`. The legacy principal/roles/op prefix is preserved so existing log scrapers keep working.
