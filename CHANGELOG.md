...
- Iceberg REST: a request whose `X-Iceberg-Access-Delegation` resolves to remote signing, which is not
  implemented, now returns the table without delegated access, as the Iceberg REST specification allows
  for this optional header. This covers `remote-signing` requested alone, and
  `vended-credentials,remote-signing` against a catalog that cannot vend credentials (for example S3 with
  `stsUnavailable: true`, or an external catalog with credential vending disabled). Previously both cases
  failed with `400 Unsupported access delegation mode: REMOTE_SIGNING`. The server logs at `INFO` when it
  falls back to no delegation.
