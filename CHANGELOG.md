...
- The `persisting` Iceberg metrics reporter no longer fails the client request when metrics persistence fails. A failed write is logged and the report dropped; previously this could surface as HTTP 500 on `POST .../metrics`.
