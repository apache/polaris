...
- Iceberg REST: a request offering several access-delegation mechanisms
  (`X-Iceberg-Access-Delegation: vended-credentials,remote-signing`) against a catalog that cannot vend
  credentials (for example S3 with `stsUnavailable: true`, or an external catalog with credential vending
  disabled) now returns the table without delegated access, as the Iceberg REST specification allows.
  Previously the server resolved the request to remote signing, which is not implemented, and failed with
  `400 Unsupported access delegation mode: REMOTE_SIGNING`. A request for `remote-signing` alone is still
  rejected.
