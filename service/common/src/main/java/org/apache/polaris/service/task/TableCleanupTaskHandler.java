...
public class TableCleanupTaskHandler implements TaskHandler {
  ...
  @Override
  public boolean handleTask(TaskEntity cleanupTask, Realm realm) {
    ...
    // Log a
    // warning and move on
    try (FileIO fileIO = fileIOSupplier.apply(cleanupTask, realm)) {
      ...
      if (createdTasks != null) {
        ...
        for (PolarisBaseEntity createdTask : createdTasks) {
          taskExecutor.addTaskHandlerContext(createdTask.getId(), realm);
        }
  ...
  private Stream<TaskEntity> getMetadataTaskStream(
      ...
      PolarisConfigurationStore configurationStore,
      Clock clock) {
    int batchSize = configurationStore.getConfiguration(realm, BATCH_SIZE_CONFIG_KEY, 10);
