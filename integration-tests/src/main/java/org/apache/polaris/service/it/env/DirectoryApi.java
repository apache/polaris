 ...
 *
 * @see PolarisClient#directoryApi(String)
 */
public class DirectoryApi extends PolarisRestApi {
  DirectoryApi(Client client, PolarisApiEndpoints endpoints, String authToken, URI uri) {
    super(client, endpoints, authToken, uri);
  }

  public void purge(String catalog, Namespace ns) {
    listDirectories(catalog, ns).forEach(t -> dropDirectory(catalog, t));
  }
  ...
  public List<TableIdentifier> listDirectories(String catalog, Namespace namespace) {
    ...
    try (Response res =
      ...
      return res.readEntity(ListDirectoriesResponse.class).getIdentifiers().stream().toList();
    }
  }
  ...
  public Directory createDirectory(
    ...
    try (Response res =
        request(
                "polaris/v1/{cat}/namespaces/{ns}/directories/",
