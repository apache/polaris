...
/**
 * Real credential-vending profile for Spark storage-name hierarchy tests.
 *
 * <p>This variant keeps the same hierarchy scenarios as {@link
 * PolarisSparkStorageNameHierarchyIntegrationTest} but enables subscoped credential vending.
 */
@ExtendWith(RustfsExtension.class)
public class PolarisSparkStorageNameHierarchyRealIntegrationTest
    extends PolarisSparkStorageNameHierarchyIntegrationTest {
  ...
  @Test
  public void testLoadTableReturnsVendedCredentialsInRealHierarchyProfile() {
    onSpark("CREATE NAMESPACE real_h");
    onSpark("CREATE TABLE real_h.events (id int, data string)");
    onSpark("INSERT INTO real_h.events VALUES (1, 'e1')");

    String principalName = client.newEntityName("real_h_delegate_user");
    String principalRoleName = client.newEntityName("real_h_delegate_role");
    ...
    LoadTableResponse response =
        delegatedCatalogApi.loadTableWithAccessDelegation(
            catalogName, TableIdentifier.of("real_h", "events"), "ALL");

    assertThat(response.credentials()).isNotEmpty();

    try (Response tableCfg = managementApi.getTableStorageConfig(catalogName, "real_h", "events")) {
