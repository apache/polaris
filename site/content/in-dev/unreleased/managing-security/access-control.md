...
| CATALOG_READ_PROPERTIES | Enables listing catalogs and reading properties of the catalog. When `ENFORCE_CATALOG_CONFIG_AUTHORIZATION` is true, catalog properties are only included in `GET /v1/config` responses for principals holding this privilege. |
