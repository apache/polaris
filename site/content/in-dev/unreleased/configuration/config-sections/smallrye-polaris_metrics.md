...
| `polaris.metrics.http-server-requests.publish-histogram` | `false` | `boolean` | Whether to publish Prometheus histogram buckets for HTTP server request metrics. <br><br>When enabled, bucket series are emitted so Prometheus compatible backends can compute  aggregable percentiles (e.g. p95, p99). Disabled by default because histogram buckets add  many additional series per unique label combination.  |
