...
|----------|---------------|------|-------------|
| `polaris.event-listener.type` |  | `string` | The type of the event listener to use. Must be a registered (`PolarisEventListener`) identifier. <br><br>_Deprecated_ since 1.5.0, use 'polaris.event-listener.types' instead, if both are set, then      polaris.event-listener.types is prioritized  |
