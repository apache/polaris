...
Master switch for handler-level idempotency. When true (and an Idempotency-Key header is present), supported handler methods (initially createTableDirect) reserve the key after authorization and rebuild the response from authoritative state on replay. May be overridden per-catalog so that a specific catalog can opt in or out of idempotency without changing the realm-wide setting.
