...
from .code_block import CodeBlock, emit_code_block, tweak_curl, tweak_docker_compose_up, tweak_spark_sql

open_pattern = re.compile("(\\s*`{3,})([a-z0-9_-]+)?\\s*")
close_pattern = re.compile("(\\s*`{3,})")


def generate_markdown_test_script(markdown_file: Path, output_file: Path) -> None:
    """
            ...
            if not code_block:
                ...
                if open_match is not None:
                    code_block = CodeBlock(
                        line_no=line_no, start_marker=open_match.group(1), code_type=open_match.group(2)
                    )
            else:
