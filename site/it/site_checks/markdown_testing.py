...
def run_test(
    ...
    with log_file.open("w", encoding="utf-8") as log_handle:
        ...
        generate_markdown_test_script(md_file, test_script)
        ...
        tee.printf("::endgroup::")
        if failed:
            summary = f"❌ Test failed for {md_file}"
        else:
            summary = f"✅ Test passed for {md_file}"
        ...
        docker_compose_info(tee, md_base_name, md_dir_name)
...
def markdown_testing(args: List[str]) -> bool:
    ...
    md_files = sorted(
        p.absolute().relative_to(site_dir)
        for p in raw_files
    )
