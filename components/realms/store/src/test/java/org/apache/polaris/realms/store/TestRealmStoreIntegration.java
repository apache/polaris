...
public class TestRealmStoreIntegration {
  ...
  public void nonSystemPersistence() {
    ...
    when(nonSystemPersistence.realmId()).thenReturn(newRealmId("nonSystemPersistence"));
  ...
  public void createUpdateDelete() {
    ...
    soft.assertThatIllegalArgumentException()
        .isThrownBy(() -> realmManagement.create(RealmId.SYSTEM))
        .withMessage("Invalid realm ID");
    soft.assertThatIllegalArgumentException()
        .isThrownBy(() -> realmManagement.create(newRealmId("::something")))
        .withMessage("Invalid realm ID");
    soft.assertThatIllegalArgumentException()
        .isThrownBy(
            () ->
                realmManagement.update(
                    something.withId(newRealmId("::something")),
                    something.withId(newRealmId("::something"))))
        .withMessage("Invalid realm ID");
    soft.assertThatIllegalArgumentException()
        .isThrownBy(() -> realmManagement.delete(something.withId(newRealmId("::something"))))
        .withMessage("Invalid realm ID");
    ...
    soft.assertThatThrownBy(
            () ->
                realmManagement.update(
                    something.withId(newRealmId("foo")), something.withId(newRealmId("foo"))))
        .isInstanceOf(RealmNotFoundException.class)
        .hasMessage("No realm with ID 'foo' exists");
    soft.assertThatThrownBy(
            () -> realmManagement.delete(something.withId(newRealmId("foo")).withStatus(PURGED)))
        .isInstanceOf(RealmNotFoundException.class)
        .hasMessage("No realm with ID 'foo' exists");
    ...
    soft.assertThatIllegalArgumentException()
        .isThrownBy(
            () ->
                realmManagement.update(
                    got,
                    RealmDefinition.builder().from(got).id(newRealmId("something-else")).build()));
