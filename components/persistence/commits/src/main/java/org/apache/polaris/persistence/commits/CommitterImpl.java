...
import org.apache.polaris.persistence.commits.retry.RetryLoop;
...
class CommitterImpl<REF_OBJ extends BaseCommitObj, RESULT>
  ...
  Optional<RESULT> commitAttempt(
    ...
    checkState(
        ...
        referenceType);
    if (refObjId.isPresent() && refObjId.orElseThrow().equals(objRef(resultObj))) {
      checkState(
    ...
    state.forAttempt.put(objRef(resultObj), resultObj);
    ...
    var resultObjRef = objRef(persistedResultObj);
    ...
    if (newReference.isEmpty()) {
      ...
      LOGGER.debug(
          "Commit change from {} with {} did not succeed, will retry, if possible",
          reference,
          resultObjRef);
    } else {
      state.allPersistedIds.add(resultObjRef);
      LOGGER.debug(
          "Commit change from {} with {} succeeded, new reference is {}",
          reference,
          resultObjRef,
          newReference.get());
    }
