...
class PersistenceMetaStoreManager implements PolarisMetaStoreManager {
  ...
  public LoadGrantsResult loadGrantsOnSecurable(
    ...
    var ms = ms(callCtx);
    return ms.loadGrants(securable.getCatalogId(), securable.getId(), securable.getTypeCode());
  }
  ...
  public LoadGrantsResult loadGrantsToGrantee(
    ...
    var ms = ms(callCtx);
    return ms.loadGrants(grantee.getCatalogId(), grantee.getId(), grantee.getTypeCode());
  }
  ...
  public EntitiesResult loadTasks(
    ...
    // TODO the following loop is NOT a "load" - it's a mutation over all tasks !!
