...
class PersistenceMetaStoreManagerFactory implements MetaStoreManagerFactory {
  ...
  PersistenceMetaStoreManagerFactory(
    ...
    realmManagement
        ...
        .forEach(
            realmDefinition -> {
              LOGGER.info("Realm registered: {}", realmDefinition);
            });
  }
  ...
  public void initializeForService(@Nonnull List<String> realmIds, @Nonnull String defaultRealm) {
    ...
    var result = bootstrapRealms(realmIds, rootCredentialsSet);
    ...
    var unusedRealmSecrets =
        realmIds.stream()
            .filter(realmId -> rootCredentialsSet.credentials().containsKey(realmId))
  ...
  public Map<String, PrincipalSecretsResult> bootstrapRealms(
    ...
    for (String realm : realms) {
      var realmId = RealmId.newRealmId(realm);
      var existing = realmManagement.get(realmId);
      if (existing.isPresent() && !existing.get().needsBootstrap()) {
        LOGGER.debug("Realm '{}' is already fully bootstrapped.", realm);
        continue;
      }
  ...
  public StorageCredentialCache getOrCreateStorageCredentialCache(RealmContext realmContext) {
    return realmState(realmContext).storageCredentialCache;
  }
  ...
  RealmState realmState(String realm) {
    var realmState = realmStateMap.get(realm);
    if (realmState != null) {
      return realmState;
    }
    ...
    synchronized (this) {
      realmState = realmStateMap.get(realm);
      if (realmState != null) {
        return realmState;
      }

      LOGGER.info("Checking realm '{}' on first use", realm);

      var realmId = RealmId.newRealmId(realm);
      var realmDesc = realmManagement.get(realmId);
      checkArgument(realmDesc.isPresent(), "Realm '%s' does not exist", realm);
      checkArgument(
      ...
      realmStateMap.put(realm, realmState);
      LOGGER.info("Done checking realm '{}'", realm);
      return realmState;
    }
  }
  ...
  private PrincipalSecretsResult bootstrapRealm(
      RealmId realmId, RootCredentialsSet rootCredentialsSet, Optional<RealmDefinition> existing) {
    LOGGER.info("Bootstrapping realm '{}' ...", realmId.id());
    ...
    var secretsResult =
        bootstrapServiceAndCreatePolarisPrincipalForRealm(
            realmId::id, metaStoreManager, metaStore, rootCredentialsSet);
    ...
    LOGGER.info("Realm '{}' has been successfully bootstrapped.", realmId.id());
  ...
  private PrincipalSecretsResult bootstrapServiceAndCreatePolarisPrincipalForRealm(
    ...
    var polarisContext = new PolarisCallContext(metaStore, diagServices, configurationStore, clock);
