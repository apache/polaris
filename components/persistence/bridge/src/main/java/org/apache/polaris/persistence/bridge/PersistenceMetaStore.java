...
class PersistenceMetaStore implements BasePersistence, IntegrationPersistence {
  ...
  private ChangeResult<MutationResults> mutationAttempt(
    ...
    for (var update : updates) {
      var entityCore = update.entity();
      var persistence = state.persistence();
      var now = persistence.currentInstant();
      LOGGER.debug("Processing update {}", update);

      var entityParentId = entityCore.getParentId();

      var entityIdKey = IndexKey.key(entityCore.getId());
      ...
      switch (update.operation()) {
        case CREATE -> {
          ...
          if (originalNameKey != null) {
            var existingRef = byName.get(originalNameKey);
          ...
          if (entityParentId != 0L && entityParentId != concern.catalogId()) {
            var parentNameKey = byId.get(IndexKey.key(entityParentId));
            if (parentNameKey == null) {
              mutationResults.entityResult(BaseResult.ReturnStatus.CATALOG_PATH_CANNOT_BE_RESOLVED);
              break;
            }
          ...
          var nameKey = identifierToIndexKey(identifier);
        ...
        case UPDATE -> {
          ...
          var originalObj =
              (ObjBase)
                  state
                      ...
                      .fetch(
                          originalRef,
                          TypeMapping.objTypeForPolarisType(entityType, entity.getSubType())
                              .targetClass());
          if (originalObj == null) {
            mutationResults.entityResult(BaseResult.ReturnStatus.ENTITY_NOT_FOUND);
            break;
          ...
          if (renameOrMove) {
            ...
            if (entityParentId != 0L && entityParentId != concern.catalogId()) {
              ...
              identifierBuilder = indexKeyToIdentifierBuilder(parentNameKey);
            }
            identifierBuilder.addElements(entity.getName());
            var newNameKey = identifierToIndexKey(identifierBuilder.build());
          ...
          var originalObj =
              (ObjBase)
                  state
                      ...
                      .fetch(
                          originalRef,
                          TypeMapping.objTypeForPolarisType(entity.getType(), entity.getSubType())
                              .targetClass());
          if (originalObj == null) {
            mutationResults.dropResult(BaseResult.ReturnStatus.ENTITY_NOT_FOUND);
            break;
  ...
  interface AclEntryHandler {
    void handle(AclObj aclObj, GrantTriplet triplet, PrivilegeSet granted);
  }

  List<PolarisGrantRecord> allGrantRecords(PolarisBaseEntity entity) {

    var catalogId = entity.getCatalogId();
    var aclName = GrantTriplet.forEntity(true, entity).toRoleName();
    ...
    collectGrantRecords(
        catalogId,
        aclName,
        (aclObj, triplet, granted) -> {
          var access = memoizedIndexedAccess.indexedAccess(triplet.catalogId(), triplet.typeCode());
          var existing = access.stableIdIndex().map(idx -> idx.get(IndexKey.key(triplet.id())));
          if (existing.isPresent()) {
            collector.handle(aclObj, triplet, granted);
          } else {
            LOGGER.trace("Not returning stale {} as it no longer exists", triplet);
          }
        });
  ...
  LoadGrantsResult loadGrants(long catalogId, long id, int entityTypeCode) {
    LOGGER.debug(
        "loadGrants for catalog {}, id {}, entityTypeCode {}", catalogId, id, entityTypeCode);
    var aclName = new GrantTriplet(true, catalogId, id, entityTypeCode).toRoleName();

    var collector = new GrantRecordsCollector(0L);
    ...
    collectGrantRecords(
        ...
        ((aclObj, triplet, granted) -> {
          var indexAccess =
              memoizedIndexedAccess.indexedAccess(triplet.catalogId(), triplet.typeCode());
          var entityOptional =
              indexAccess.byId(triplet.id()).map(o -> mapToEntity(o, triplet.catalogId()));
  ...
  private void collectGrantRecords(
    ...
    LOGGER.debug("Collecting grant records for ACL '{}' from '{}'", aclName, refName);
    ...
    if (head.isPresent()) {
      ...
      LOGGER
          .atTrace()
          .setMessage("GrantsObj ACL keys: {}")
          .addArgument(
              () -> {
                var keys = new ArrayList<>();
                securablesIndex.iterator().forEachRemaining(e -> keys.add(e.getKey().toString()));
                return keys;
              })
          .log();

      LOGGER.debug("Checking grant records for key {}", aclName);
              ...
              aclObj -> {
                ...
                acl.forEach(
                    (role, entry) -> {
                      ...
                      aclEntryConsumer.handle(aclObj, triplet, entry.granted());
                    });
              });
    }
  }
  ...
  static class GrantRecordsCollector implements AclEntryHandler {
    ...
    @Override
    public void handle(AclObj aclObj, GrantTriplet triplet, PrivilegeSet granted) {
      for (var privilege : granted) {
        var polarisPrivilege = PolarisPrivilege.valueOf(privilege.name()).getCode();
        grantRecords.add(
            triplet.securable()
  ...
  private <O extends GrantsObj, B extends GrantsObj.Builder<O, B>> void doPersistGrantsOrRevokes(
      String refName, Supplier<B> builder, boolean grant, Grant... grants) {
    LOGGER.debug(
        "Persisting {} on '{}' for '{}'",
        grant ? "grants" : "revokes",
        refName,
          ...
          .commitRuntimeException(
              new CommitRetryable<>() {
                ...
                public Optional<GrantsObj> attempt(
                  ...
                  for (var g : grants) {
                    var securableIsGrantee = g.securable().getType().isGrantee();
                    LOGGER.trace("Adding grant (grantee={}): {}", securableIsGrantee, g);
                    var privilege = privileges.byName(g.privilege().name());
                    processGrant(
                        state,
                        g.securable(),
                        GrantTriplet.forEntity(false, g.grantee()),
                        securablesIndex,
                        privilege,
                        grant);
                    processGrant(
                        state,
                        g.grantee(),
                        GrantTriplet.forEntity(true, g.securable()),
                        securablesIndex,
                        privilege,
                        grant);
                  }
                ...
                private void processGrant(
                    ...
                    PolarisEntityCore aclEntity,
                    GrantTriplet grantee,
                    UpdatableIndex<ObjRef> securablesIndex,
                    Privilege privilege,
                    boolean grant) {

                  var aclTriplet = GrantTriplet.forEntity(true, aclEntity);
                  ...
                  var aclObjBuilder =
                      aclObjOptional
                          ...
                          .id(persistence.generateId())
                          .securableId(aclEntity.getId())
                          .securableTypeCode(aclEntity.getTypeCode());
                  ...
                  state.writeOrReplace("acl-" + aclEntity.getId(), aclObj);
    ...
    return performPrincipalChange(
        ...
        (state, ref, byName, byId, byClientId) -> {
          ...
          if (existingPrincipal.isPresent()) {
            return new ChangeResult.NoChange<>(
                new CreatePrincipalResult(BaseResult.ReturnStatus.ENTITY_ALREADY_EXISTS, null));
          }
  ...
  protected PrincipalSecretsGenerator secretsGenerator(
    ...
    if (rootCredentialsSet != null) {
      var realmId = this.persistence.realmId().id();
