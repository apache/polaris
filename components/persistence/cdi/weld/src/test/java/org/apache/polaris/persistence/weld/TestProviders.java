...
public class TestProviders {
  ...
  public void checkProviders() {
    ...
    soft.assertThat(backend.name()).isEqualTo("InMemory");

    var realmManagement = weld.select(RealmManagement.class).get();
    soft.assertThat(realmManagement.get(newRealmId("fooBar"))).isEmpty();

    var systemPersistence = weld.select(Persistence.class, new SystemPersistence.Literal()).get();
    soft.assertThat(systemPersistence.realmId()).isEqualTo(RealmId.SYSTEM);
