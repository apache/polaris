...
import static org.apache.polaris.persistence.api.index.IndexKey.deserializeKey;
...
final class IndexImpl<V> implements IndexSpi<V> {
  ...
  private static final ThreadLocal<ByteBuffer> SCRATCH_KEY_BUFFER = new ThreadLocal<>();

  private static ByteBuffer scratchKeyBuffer() {
    var buf = SCRATCH_KEY_BUFFER.get();
    if (buf == null) {
      buf = newKeyBuffer();
      SCRATCH_KEY_BUFFER.set(buf);
    } else {
      buf.clear();
    }
    return buf;
  ...
  public String toString() {
    ...
    return "StoreIndexImpl{size=" + elements.size() + ", first=" + fk + ", last=" + lk + "}";
  }
  ...
  private final class LazyIndexElement extends AbstractIndexElement<V> {
    ...
    @Override
    public boolean deserializedAndUnmodified() {
      return true;
    }
