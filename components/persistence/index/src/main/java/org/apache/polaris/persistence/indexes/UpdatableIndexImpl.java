...
import static org.apache.polaris.persistence.api.index.IndexStripe.indexStripe;
import static org.apache.polaris.persistence.api.obj.ObjRef.objRef;
import static org.apache.polaris.persistence.indexes.IndexStripeObj.indexStripeObj;
...
final class UpdatableIndexImpl<V> extends AbstractLayeredIndexImpl<V> implements UpdatableIndex<V> {
    ...
    for (var elemIter = embedded.elementIterator(); elemIter.hasNext(); ) {
      ...
      if (value == null) {
        if (reference.contains(key)) {
          // 'key' is being removed, only keep it in the embedded index, if it is needed to shadow
  ...
  private void spillOutEmbedded(
    ...
    var newEmbedded = updateAffectedStripes(mutableReference);

    // Set the new embedded index
    indexedBuilder.embedded(newEmbedded.serialize());
  ...
  private IndexSpi<V> updateAffectedStripes(IndexSpi<V> mutableReference) {
    var newEmbedded = newStoreIndex(serializer);
    for (var elemIter = embedded.elementIterator(); elemIter.hasNext(); ) {
  ...
  private List<IndexSpi<V>> collectSurvivingStripes(IndexSpi<V> mutableReference) {
    ...
    for (var stripe : mutableReference.stripes()) {
      ...
      if (stripe.hasElements()) {
        ...
        if (desiredSplits > 1) {
          // Stripe became too big, needs to be split further
          LOGGER.debug(
  ...
  public boolean remove(@Nonnull IndexKey key) {
    ...
    if (updExisting != null && updExisting.getValue() == null) {
      // removal sentinel already present, do nothing
      return false;
    }
    ...
    if (updExisting != null) {
      // Key does not exist in the reference index, just remove it
      embedded.remove(key);
      return true;
    }
