...
public interface CatalogStateObj extends ContainerObj {
   ...
   * index. Whether a change is contained here, depends on functional, not technical requirements.
   * Changes that affect many entities (bulk updates) may also be only recorded once, for example
   * renaming a namespace with implicit rename of the contained entities might only be mentioned
   * once for the rename of the namespace, not for all contained entities.
   */
  IndexContainer<Change> changes();
