...
public class TestDistributedInvalidations {
  ...
  public void setup() {
    ...
    var emitter1 = new AtomicReference<DistributedCacheInvalidation>();
    var emitter2 = new AtomicReference<DistributedCacheInvalidation>();

    sender1 = spy(delegate(emitter2::get));
    sender2 = spy(delegate(emitter1::get));

    distributed1 =
        PersistenceCaches.wrapBackendForDistributedUsage(
  ...
  protected static DistributedCacheInvalidation delegate(
    ...
    return new DistributedCacheInvalidation() {
      ...
      public void evictObj(@Nonnull RealmId realmId, @Nonnull ObjRef objRef) {
        invalidation.get().evictObj(realmId, objRef);
      }

      @Override
      public void evictReference(@Nonnull RealmId realmId, @Nonnull String refName) {
        invalidation.get().evictReference(realmId, refName);
      }
    };
  }
}
