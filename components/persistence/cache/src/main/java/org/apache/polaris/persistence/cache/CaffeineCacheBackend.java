...
import static org.apache.polaris.persistence.api.obj.ObjType.CACHE_UNLIMITED;
import static org.apache.polaris.persistence.api.obj.ObjType.NOT_CACHED;
import static org.apache.polaris.persistence.api.obj.ObjTypes.objTypeById;
import static org.apache.polaris.persistence.cache.CacheConfig.DEFAULT_REFERENCE_TTL;
import static org.apache.polaris.persistence.cache.CacheSizing.DEFAULT_HEAP_FRACTION;
import static org.apache.polaris.persistence.cache.CaffeineCacheBackend.CacheKeyValue.KIND_FLAG_CREATED;
import static org.apache.polaris.persistence.cache.CaffeineCacheBackend.CacheKeyValue.KIND_FLAG_NUM_PARTS;
import static org.apache.polaris.persistence.cache.CaffeineCacheBackend.CacheKeyValue.KIND_FLAG_OBJ_ID;
import static org.apache.polaris.persistence.cache.CaffeineCacheBackend.CacheKeyValue.KIND_FLAG_VERSION;
import static org.apache.polaris.persistence.cache.CaffeineCacheBackend.CacheKeyValue.KIND_REFERENCE;
import static org.apache.polaris.tools.varint.VarInt.putVarInt;
import static org.apache.polaris.tools.varint.VarInt.readVarInt;
import static org.apache.polaris.tools.varint.VarInt.varIntLen;
...
class CaffeineCacheBackend implements CacheBackend {
  ...
  CaffeineCacheBackend(CacheConfig config, Optional<MeterRegistry> meterRegistry) {
    ...
    meterRegistry.ifPresent(
        reg -> {
          ...
          reg.gauge(
              "cache_capacity", singletonList(Tag.of("cache", CACHE_NAME)), "", x -> capacityBytes);
        });
  ...
  public void clear(@Nonnull RealmId realmId) {
    var t = realmId.id();
    cache.asMap().keySet().removeIf(k -> k.realmId.equals(t));
  }
  ...
  static CacheKeyValue cacheKeyValueObj(
      @Nonnull RealmId realmId, @Nonnull Obj obj, long expiresAtNanos) {
    var serialized = serializeObj(obj);
    return new CacheKeyValue(realmId.id(), cacheKeyObj(obj), expiresAtNanos, serialized);
  }

  @VisibleForTesting
  static CacheKeyValue cacheKeyValueObjRead(@Nonnull RealmId realmId, @Nonnull ObjRef id) {
    return new CacheKeyValue(realmId.id(), cacheKeyObjId(id), 0L, null);
  }

  @VisibleForTesting
  static CacheKeyValue cacheKeyValueReference(
      RealmId realmId, Reference reference, long expiresAtNanos) {
  ...
  @VisibleForTesting
  static CacheKeyValue cacheKeyValueReferenceRead(@Nonnull RealmId realmId, @Nonnull String name) {
    return new CacheKeyValue(realmId.id(), cacheKeyReference(name), 0L, null);
  }

  @VisibleForTesting
  static CacheKeyValue cacheKeyValueNegative(
      @Nonnull RealmId realmId, @Nonnull byte[] key, long expiresAtNanosEpoch) {
    return new CacheKeyValue(realmId.id(), key, expiresAtNanosEpoch, null);
  }
  ...
  static final class CacheKeyValue {
    ...
    public String toString() {
      ...
      if (kind == KIND_REFERENCE) {
        var refertenceName = new String(key, 1, key.length - 1, UTF_8);
        sb.append("reference:").append(refertenceName);
      } else {
        var relevantLen = readVarInt(buf);
        var id = buf.getLong();
