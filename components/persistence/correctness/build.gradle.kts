...
description = "Polaris persistence correctness tests"

dependencies {
  testFixturesApi(project(":polaris-persistence-api"))
  testFixturesApi(project(":polaris-persistence-commits"))
    ...
    dbs.forEach { prjDbs ->
      ...
      prjDbs.value.forEach { db ->
        ...
        register<JvmTestSuite>("correctness${dbTaskName}Test") {
          dependencies {
            runtimeOnly(project(":polaris-persistence-$prj"))
            implementation(testFixtures(project(":polaris-persistence-$prj")))
          }
