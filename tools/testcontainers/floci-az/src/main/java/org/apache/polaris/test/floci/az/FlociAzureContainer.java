...
public class FlociAzureContainer extends GenericContainer<FlociAzureContainer>
    implements FlociAzureAccess, AutoCloseable {
  ...
  public FlociAzureContainer() {
    this(null, null, null, null);
  }

  public FlociAzureContainer(String image, String storageContainer, String account, String secret) {
    super(
        ContainerSpecHelper.containerSpecHelper("floci-az", FlociAzureContainer.class)
            .dockerImageName(image));
    this.storageContainer =
        storageContainer != null ? storageContainer : randomString("filesystem");
    this.account = account != null ? account : DEFAULT_ACCOUNT;
    this.accountFq = this.account + ".dfs.core.windows.net";
    this.secret = secret != null ? secret : DEFAULT_ACCOUNT_KEY;
    this.secretBase64 = secret != null ? base64(secret) : DEFAULT_ACCOUNT_KEY;

    addExposedPort(DEFAULT_PORT);
    setWaitStrategy(Wait.forHttp("/_floci/health").forPort(DEFAULT_PORT));
    if (account != null || secret != null) {
      addEnv("AZURE_USERNAME", this.account);
      addEnv("AZURE_PASSWORD", this.secret);
    }
  }
