...
public class JavaPoolAsyncExec implements AsyncExec, AutoCloseable {
  ...
  public <R> Cancelable<R> schedule(Callable<R> callable, Duration delay) {
    ...
    delayed(cf, Math.max(delay.toMillis(), 0L));
  ...
  void immediate(CancelableFuture<?> future) {
    executorService.submit(future);
  }

  @SuppressWarnings("FutureReturnValueIgnored")
  void delayed(CancelableFuture<?> future, long delay) {
    scheduler.schedule(() -> immediate(future), delay, MILLISECONDS);
  }
