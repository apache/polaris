...
class MaintenanceServiceImpl implements MaintenanceService {
    ...
    try {
      try {
        ...
        try (var refHandler =
          ...
          if (!allRetained.withinExpectedFpp(expectFpp)) {
            var warn =
      ...
      systemPersistence.write(
          ImmutableMaintenanceRunObj.builder().from(runObj).runInformation(info).build(),
  ...
  private void updateRealmsAsPurged(
    ...
    // Update the realm status of the realms that were specified to be purged to "PURGED", if no
  ...
  private AllRetained constructAllRetained(MaintenanceConfig effectiveConfig) {
    ...
    for (var lastRunIter =
      ...
      if (refs > expectedReferenceCount) {
        // Add 10% to account for new created references
        expectedReferenceCount =
            (long)
                (refs
                    * effectiveConfig
      ...
      if (objs > expectedObjCount) {
        // Add 10% to account for new created objects
        expectedObjCount =
            (long)
                (objs
                    * effectiveConfig
  ...
  static long bloomFilterBytes(long elements, double fpp) {
    var bits = (long) (-elements * Math.log(fpp) / (Math.log(2) * Math.log(2)));
