...
public class PolarisJdbcBasePersistenceImpl implements BasePersistence, IntegrationPersistence {
    ...
    public void writeEntity(
        ...
        if (originalEntity == null) {
            query = JdbcCrudQueryGenerator.generateInsertQuery(modelEntity, "polaris_schema.entities");
        } else {
            ...
            query = JdbcCrudQueryGenerator.generateUpdateQuery(modelEntity, params, "polaris_schema.entities");
        }
        System.out.println("Executing query: " + query);
        int x = datasourceOperations.executeUpdate(query);
                            ...
                            if (originalEntities == null || originalEntities.get(i) == null) {
                                ...
                                query =
                                        JdbcCrudQueryGenerator.generateUpdateQuery(
                                                modelEntity, params, "entities");
                            }
                            int x = datasourceOperations.executeUpdate(query, statement);
                            if (x == 0) {
                                if (originalEntities == null || originalEntities.get(i) == null) {
                                    // bad interface.
                                    throw new EntityAlreadyExistsException(entity);
    ...
    public void writeToGrantRecords(PolarisCallContext callCtx, PolarisGrantRecord grantRec) {
        ...
        String query =
                JdbcCrudQueryGenerator.generateInsertQuery(modelGrantRecord, "POLARIS_SCHEMA.GRANT_RECORDS");
        System.out.println("Executing query: " + query);
    ...
    public void deleteEntity(PolarisCallContext callCtx, PolarisBaseEntity entity) {
        ...
        String query = JdbcCrudQueryGenerator.generateDeleteQuery(params, "POLARIS_SCHEMA.entities");
        datasourceOperations.executeUpdate(query);
    }
    ...
    public void deleteFromGrantRecords(PolarisCallContext callCtx, PolarisGrantRecord grantRec) {
        ModelGrantRecord modelGrantRecord = ModelGrantRecord.fromGrantRecord(grantRec);
        String query =
                JdbcCrudQueryGenerator.generateDeleteQuery(modelGrantRecord, "POLARIS_SCHEMA.grant_records");
        datasourceOperations.executeUpdate(query);
    }
    ...
    public void deleteAll(PolarisCallContext callCtx) {
        datasourceOperations.executeUpdate("DELETE FROM POLARIS_SCHEMA.grant_records where (1 = 1)");
        datasourceOperations.executeUpdate("DELETE FROM POLARIS_SCHEMA.entities where (1 = 1)");
        datasourceOperations.executeUpdate("DELETE FROM POLARIS_SCHEMA.principal_authentication_data where (1 = 1)");
    ...
    public PolarisPrincipalSecrets loadPrincipalSecrets(
        ...
        return results == null || results.isEmpty()
                ? null
                : results.stream().map(ModelPrincipalAuthenticationData::toPrincipalSecrets).toList().get(0);
    }
    ...
    public PolarisPrincipalSecrets generateNewPrincipalSecrets(
        ...
        String query =
                JdbcCrudQueryGenerator.generateInsertQuery(
                        lookupPrincipalSecrets, "POLARIS_SCHEMA.principal_authentication_data");
        datasourceOperations.executeUpdate(query);
    ...
    public void deletePrincipalSecrets(
        ...
        String query = JdbcCrudQueryGenerator.generateDeleteQuery(params, "POLARIS_SCHEMA.principal_authentication_data");
        datasourceOperations.executeUpdate(query);
    }
