...
public interface TasksConfiguration {
  ...
  /**
   * Running tasks update regularly update the task store with their state, this parameter defines
   * the interval of these updates.
   */
  ...
  /**
   * Interval at which the state of task that is running on another node is being polled.
   *
   * <p>Call sites that subscribe to the result of a task, which runs on a different node, need the
   * result.
   *
  ...
  /**
   * New submitted that are scheduled with this interval will be scheduled for noe-local execution,
