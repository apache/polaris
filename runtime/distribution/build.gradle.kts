    ...
    contents {
      ...
      into("sbom") {
        ...
        from(configurations["adminSbom"]) {
          eachFile {
            name = name.replace(Regex("^bom[.](xml|json)$"), "polaris-admin-${project.version}.cyclonedx.$1")
          }
        }
        from(configurations["serverSbom"]) {
          eachFile {
            name = name.replace(Regex("^bom[.](xml|json)$"), "polaris-server-${project.version}.cyclonedx.$1")
          }
        }
      }
