...
public class S3RemoteSigningCatalogAdapter
  ...
  private Response withCatalog(
    ...
    String catalogName = prefixParser.prefixToCatalogName(callContext.getRealmContext(), prefix);
    try (S3RemoteSigningCatalogHandler handler = newCatalogHandler(securityContext, catalogName)) {
  ...
  @VisibleForTesting
  S3RemoteSigningCatalogHandler newCatalogHandler(
