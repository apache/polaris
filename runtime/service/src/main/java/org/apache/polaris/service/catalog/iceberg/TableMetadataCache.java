...
/**
 * Process-wide cache of table metadata JSON documents keyed by realm, catalog and metadata file
 * location. Metadata files are immutable once written, so an entry never becomes stale and the
 * location acts as a content address. Callers resolve and validate storage access for the location
 * before consulting the cache, so a hit only skips the object-storage read, never any
 * authorization; entries are additionally scoped to the realm and catalog that populated them. The
 * raw JSON is cached rather than parsed {@link org.apache.iceberg.TableMetadata} objects because
 * {@code TableMetadata} lazily materializes some fields and is not safe to share across request
 * threads, and raw documents allow bounding the cache by size. Entry weights estimate heap usage
 * (UTF-16 characters plus per-entry map and object overhead); the configured budget is an
 * approximate bound because eviction may briefly lag writes.
 */
@ApplicationScoped
public class TableMetadataCache {

  /** Scopes a metadata document to the realm and catalog that is allowed to read it. */
  public record Key(String realmId, long catalogId, String metadataLocation) {}
  ...
  private static String read(FileIO fileIO, String metadataLocation) {
    ...
    try (InputStream stream =
      ...
      throw new UncheckedIOException("Failed to read metadata file " + metadataLocation, e);
    }
  }
}
