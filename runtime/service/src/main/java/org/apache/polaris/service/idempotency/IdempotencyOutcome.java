 ...
 *   <li>{@link New} — no prior record exists for this key: the handler performs the operation and
 *       then records the outcome. The binding (key, operation, and the single binding hash) travels
 *       inside so the record and race-resolution steps need no re-hashing.
...
public sealed interface IdempotencyOutcome
    permits IdempotencyOutcome.Disabled, IdempotencyOutcome.New, IdempotencyOutcome.Duplicate {
  ...
  /**
   * No record exists yet for this key, so this caller may proceed. Carries the binding so {@link
   ...
   */
  record New(UUID idempotencyKey, IdempotentOperation operation, String bindingHash)
