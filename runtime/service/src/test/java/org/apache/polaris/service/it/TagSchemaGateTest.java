...
public class TagSchemaGateTest {
  ...
  public void testAssignTagRejectedBelowSchemaV7() {
    ...
    TagAttachmentTarget target = TagAttachmentTarget.builder(TargetType.CATALOG).build();
    try (Response res =
        tagApi.assignTagResponse(currentCatalogName, tagName, target, List.of("public"))) {
  ...
  public void testDetachAllDropRejectedBelowSchemaV7() {
    ...
    try (Response res =
      ...
      // Every 5xx answers the fixed literal InternalServerError, regardless of the exception
      // class that raised it: the contract treats a server failure as opaque.
      Assertions.assertThat(body).contains("\"type\":\"InternalServerError\"").contains("schema");
      Assertions.assertThat(body).contains("schema version").contains("7");
    }
  }
