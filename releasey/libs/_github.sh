...
function check_github_checks_passed() {
  ...
  local num_invalid_checks_retrieval_command="curl -s -H \"Authorization: Bearer ${GITHUB_TOKEN}\" -H \"Accept: application/vnd.github+json\" -H \"X-GitHub-Api-Version: 2022-11-28\" https://api.github.com/repos/${repo_info}/commits/${commit_sha}/check-runs | jq -r '[.check_runs[].conclusion | select(. != null and . != \"success\" and . != \"skipped\")] | length'"
